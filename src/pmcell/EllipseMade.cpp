...
void EllipseMade::ellip_generation(ifstream &infile,string output_file,string data_file,double mod)
{
    ...
    if (a_min>a_max)
    {
        std::cout << string("输入的椭球长轴最小值大于最大值！") << endl;
        exit(0);
    }
...
///
///
void EllipseMade::ellip_generation(double* in_para,string output_file,string data_file,double mod)
{
    struct  point_type
    {
        double	x;
